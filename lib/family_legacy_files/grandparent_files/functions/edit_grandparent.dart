...
class EditGrandparent extends StatefulWidget {
  final String userID,
      famLegacyID,
      grandparentID,
  ...
  const EditGrandparent({
    super.key,
    required this.userID,
    required this.famLegacyID,
    required this.grandparentID,
    required this.selectedGrandparentID,
    required this.selectedSpouseID,
  });

  @override
  State<EditGrandparent> createState() => _EditGrandparentState();
}

class _EditGrandparentState extends State<EditGrandparent> {
  ...
  void initState() {
    super.initState();
    selectedParentID = widget.selectedGrandparentID;
    if (widget.selectedSpouseID.isNotEmpty) {
      selectedSpouseID = widget.selectedSpouseID;
      _fetchSelectedSpouseData(widget.selectedSpouseID);
    } else {
      selectedSpouseID = 'No spouse';
      hasSpouse = true;
    }

    _fetchSelectedGrandparentData(widget.selectedGrandparentID);
  }

  Future<void> _fetchSelectedGrandparentData(String id) async {
  ...
  Future _updateChildAndSpouse() async {
    try {
      if (selectedParent != null) {
        if (hasSpouse && selectedSpouse != null) {
          await updateGrandparent(
              widget.famLegacyID,
        ...
        showDialog(
          ...
          builder: (BuildContext context) {
            return const AlertBox(
              type: 'Update grandparent successful',
              object: 'Successfully update grandparent',
            );
          },
        ).then((_) {
      ...
      showDialog(
        ...
        builder: (BuildContext context) {
          return AlertBox(
            type: 'Failed to update grandparent',
            object: 'Error: $e',
          );
        },
      );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(title: const Text('Edit Grandparent Details')),
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    DropdownButtonFormField<String>(
                      ...
                      onChanged: (value) {
                        ...
                        if (selectedParentID.isNotEmpty) {
                          _fetchSelectedGrandparentData(selectedParentID);
                        }
                      },
                      validator: (value) {
                    ...
                    SizedBox(height: _heightSize),
                    if (selectedParent != null) ...[
                      const Text('Current Details of the child:'),
                      SizedBox(height: _heightSize),
                      Text('Name: ${selectedParent!.name}'),
                      SizedBox(height: _heightSize),
                      Text('Birth Order: ${selectedParent!.birthOrder}'),
                      SizedBox(height: _heightSize),
                      ...
                      if (selectedSpouse != null) ...[
                        const Text(
                          'Currrent Details of the spouse:',
                        ),
                        SizedBox(height: _heightSize),
                        Text(
